<template>
  <div class="PayBottom_components" v-show="!isFirstPage">
    ...
    <div @click="handleClick" class="button_box flex f-y-c f-x-c">
      <img src="../assets/PayLock_button.png" alt="" />
    </div>
  </div>
</template>
  ...
  methods: {
    scroll() {
      window.addEventListener('scroll', () => {
        const height = document.getElementsByClassName('PayLock_components')[0].clientHeight;
        const lockScrollTop = document.getElementsByClassName('PayLock_components')[0].offsetTop;

        const scrollTop = document.getElementsByTagName('html')[0].scrollTop;

        if (scrollTop > height + lockScrollTop) {
